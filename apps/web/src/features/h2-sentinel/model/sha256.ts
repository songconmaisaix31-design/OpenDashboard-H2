...
/** Incremental SHA-256 keeps large CSVs chunk-bounded in browser memory. */
export class H2Sha256 {
  ...
  private compress(block: Uint8Array): void {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = ((h ?? 0) + sum1 + choice + (ROUND_CONSTANTS[index] ?? 0) + (words[index] ?? 0)) >>> 0
...
export function h2Sha256Hex(bytes: Uint8Array): string {
  return new H2Sha256().update(bytes).digestHex()
}
