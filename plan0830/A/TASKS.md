...
| A-P0-2 | 报警弱特征融合 | P0 | 进行中 | 三绿+169 pytest+子类一致率 | **会话1 完成** @407ed1b：共现核实（`tools/verify_alarm_cooccurrence.py`）修正假设表——严格一对一共现，C02←ELZ_POWER_DEVIATION（原判 CAPACITY_SYNC_WARN 零共现），C04/C05/C06/C07 收敛唯一码，余 7 关联码起点窗零共现留会话2；`alarm_features.py` 两簇+集合语义+H2_ALARM_LOG_PATH 惰性单例；aggregator 置信 +0.02（上限 0.99，无码不罚，事件集合不变）；三绿复验 F1=0.9718 持平/尺子 77 窗 0 FP/哨兵 Δ=0.012/pytest 全绿（+5 测）；真实数据端到端命中 28/350 与离线核实逐一吻合（train+val：C01 4/C02 11/C03 8/C04 5/C05 4/C06 7/C07 2）。**待会话2**：C04/C05 子类消歧（按事件全程窗再核 7 个零共现码）+子类一致率报告。alarmFeatures 参数随 CR-B1 v6 收口 |
