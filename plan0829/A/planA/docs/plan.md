...
| T02 | [A2] | P0-4：`validation/normal-context-regression.mjs` + 分列误报率 + 基线冻结 + 接入 check-all | T01 | A-1 门禁（`../../TASKS.md`） | ☐ |
| T03a | [A1] | P0-5 判据侧：`detection/rules.py` C05/C07 前瞻判据（消耗速率/备用差值外推） | T02 | N05/N07 不误报；判据三要素入校准块 | ☐ |
| T03b | [A2] | P0-5 指标侧：`evaluate.mjs` 新增 `lead_time_minutes` 与 10 分钟检出率 | T02 | A-2 门禁指标可产出（口径=ADR-004/IF-4） | ☐ |
| T04-T06 | [A1] | P1-1 三连：C03 → C05 → C06 去签名带（逐类独立 commit） | T03a+b | 每类四重门禁绿 + TRAIN 40 全命中 | ☐ |
...
| T12 | [A3] | P1-8：根因数据驱动文本 + IF-2 冻结 | T10 | A-6 门禁 | ☐ |
| T13 | [A3] | 每个 M-Gate 出评审报告 `docs/reviews/gate-<n>.md` + 校准记录块补全督促 | T02 起 | 报告与门禁证据齐备 | ☐ |
