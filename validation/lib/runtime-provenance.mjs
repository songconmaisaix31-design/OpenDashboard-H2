...
const unsafeControlText = /(?:并非|不是|否认|无需|不需|不须|不必|免于|绕过).{0,12}(?:人工|确认)|(?:人工确认|确认).{0,8}(?:并非|不是|不再是).{0,8}(?:必需|必须|必要|条件)|(?:自动|直接).{0,12}(?:执行|控制|下发|发送|操作)|(?:系统|应用).{0,12}(?<![不未无])(?:可以|可|可能|能够|将会|有权|拥有|具备|获授权|被授权|获准|被允许|允许|获许可).{0,12}(?:执行|控制|下发|发送).{0,12}(?:设备|指令|命令|操作)?|(?:设备(?:指令|命令|操作|控制)|控制(?:指令|命令|操作)?).{0,8}(?<![不未无])(?:可以|可|能够|将会).{0,4}由(?:系统|应用).{0,8}(?:执行|控制|下发|发送)|(?:无需|不经|绕过)人工确认/u
