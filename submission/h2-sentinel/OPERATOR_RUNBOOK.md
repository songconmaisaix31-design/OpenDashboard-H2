...
RUN2 补充观察（处置详表见上引闭环表 F4-F6）：

- **导入后 1-3 分钟为高负载窗口**（98 事件证据链与图表首渲染）：页面
  点击或截图可能短暂无响应，等待加载完成即可，勿判定挂死重启。
- **共享机上多实例并行时**，停止服务须按工作目录/路径限定进程，仅终止
  自己启动的进程树；宽匹配可能误伤其他实例。
- **第二个浏览器客户端**在上述高负载窗口并发访问可能长时间停在
  「正在加载可核验运行…」；单客户端顺序操作不受影响。
