...
| C15 | “P2 source implements ordered, bounded, hash-checked upload sessions and the Web adapter calls them for large files.” | Reviewed source fact | Dependency commits plus coordinator integration; source and enablement do not alone prove a completed full-file launcher/Web run. |
| C16 | “The standard Local path imports the full 236991870-byte training file.” | Pending final-candidate evidence | Strict `H2_STREAMING_IMPORT_ENABLED=true` opt-in is implemented in `40b3d391`; requires a fresh final-SHA launcher/Web run and exact external size/hash evidence. |
